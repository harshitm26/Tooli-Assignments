...
\node[ball,text width=3cm,fill=purple!20] (all) at (6,0) {Animal};

% two circles showing split of p{cancer} and p{~cancer}
\node[ball,fill=red!30,text width=2.8cm,anchor=base] (pcan) at (3.5,-6) {Unicellular};
\node[ball,fill=blue!40,text width=2.8cm,anchor=base] (pncan) at (8.5,-6)
   {Mammals};

% arrows showing split from all women to cancer and ~cancer
\draw[->,thick,draw=blue!50] (all.south) to [out=270,in=90] (pcan.north);
